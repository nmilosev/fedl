...
if 'USE_GPUS' in os.environ and os.environ['USE_GPUS'] == '1':
    ...
    print('Not using CUDA because of USE_GPUS=0 parameter!')
    NUM_GPUS = 1 # setting NUM_GPUS to 1 for batch_size calculations 
    DISTRIBUTED_STRATEGY = tf.distribute.OneDeviceStrategy(device="/cpu:2")
...
def get_model():
    ...
    for filter_size in feature_fusion_block_filters:
        ...
        tiled_gamma = tf.keras.layers.RepeatVector(VIDEO_SIZE[0] // 8 * VIDEO_SIZE[1] // 8)(gamma)
        tiled_gamma = tf.reshape(tiled_gamma, (-1, VIDEO_SIZE[0] // 8, VIDEO_SIZE[1] // 8, filter_size))
        video_output = tf.keras.layers.Multiply()([video_output, tiled_gamma])

        beta = tf.keras.layers.Dense(filter_size)(audio_output)
        tiled_beta = tf.keras.layers.RepeatVector(VIDEO_SIZE[0] // 8 * VIDEO_SIZE[1] // 8)(beta)
        tiled_beta = tf.reshape(tiled_beta, (-1, VIDEO_SIZE[0] // 8, VIDEO_SIZE[1] // 8, filter_size))
    ...
    model = tf.keras.models.Model(
        inputs=[audio_input_tensor, video_input],
    ...
    for split in ['train', 'val', 'test']:
        ...
        for index in range(infos_len):
            ...
            if recreate or not os.path.exists(all_path):
                ...
                waveform, _ = librosa.load(info['waveform_path'], sr=DISCO_SAMPLING_RATE)
                ...
                resize_errors.append(np.abs(np.sum(density_map) - np.sum(resized_density_map)))
...
class AVCCClient(fl.client.NumPyClient):
    def __init__(self, *args, **kwargs):
        ...
        self.model, self.train_sequence, self.test_sequence, self.val_sequence = train_backbone(0) # 0 epochs, only to init model
